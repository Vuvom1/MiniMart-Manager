...
class CustomerController {
  all_get = async (req, res) => {
    try {
      ...
      const usersWithCustomer = await Promise.all(
  ...
  statisticByDate = async () => {
    try {
      ...
      const todayNewCustomers = await Customer.countDocuments({
        createdAt: {
          $gte: DateUtil.getStartOfToday(),
          $lt: DateUtil.getCurrentDate(),
        },
      });

      const yesterdayNewCustomers = await Customer.countDocuments({
        createdAt: {
          $gte: DateUtil.getStartOfYesterday(),
          $lt: DateUtil.getEndOfYesterday(),
        },
      });

      const totalCustomerComparison =
        yesterdayTotalCustomers === 0
          ? todayTotalCustomers
          : todayTotalCustomers / yesterdayTotalCustomers;
      const totalCustomerPercentage = totalCustomerComparison * 100;

      const newCustomerComparison =
  ...
  statisticByMonth = async () => {
    try {
      ...
      const currentMonthNewCustomers = await Customer.countDocuments({
        createdAt: {
          $gte: DateUtil.getStartOfCurrentMonth(),
          $lt: DateUtil.getCurrentDate(),
        },
      });
      const previousMonthNewCustomers = await Customer.countDocuments({
        createdAt: {
          $gte: DateUtil.getStartOfLastMonth(),
          $lt: previousMonthEnd,
        },
      });

      const totalCustomerComparison =
        previousMonthTotalCustomers === 0
          ? currentMonthTotalCustomers
          : currentMonthTotalCustomers / previousMonthTotalCustomers;
      const totalCustomerPercentage = totalCustomerComparison * 100;

      const newCustomerComparison =
        previousMonthNewCustomers === 0
          ? currentMonthNewCustomers
          : currentMonthNewCustomers / previousMonthNewCustomers;
      const newCustomerPercentage = newCustomerComparison * 100;
  ...
  statisticByYear = async () => {
    try {
      ...
      const currentYearNewCustomers = await Customer.countDocuments({
        createdAt: {
          $gte: DateUtil.getStartOfCurrentYear(),
          $lt: DateUtil.getCurrentDate(),
        },
      });
      const previousYearNewCustomers = await Customer.countDocuments({
        createdAt: {
          $gte: DateUtil.getStartOfLastYear(),
          $lt: DateUtil.getEndOfLastYear(),
        },
      });

      const totalCustomerComparison =
        previousYearTotalCustomers === 0
          ? currentYearTotalCustomers
          : currentYearTotalCustomers / previousYearTotalCustomers;
      const totalCustomerPercentage = totalCustomerComparison * 100;

      const newCustomerComparison =
        previousYearNewCustomers === 0
          ? currentYearNewCustomers
          : currentYearNewCustomers / previousYearNewCustomers;
      const newCustomerPercentage = newCustomerComparison * 100;
