...
import { addPromotion, editPromotion, getPromotionById } from "../../services/api/PromotionApi";
...
function EditPromotion() {
    ...
    const fetchPromotionById = async (id: string) => {
        setLoading(true)
        try {
            ...
            setMaxUsage(data.maxUsage);
            setDescription(data.description);
            setSelectedPromotionType(data.type);
            setDiscountType(data.discountType);
            setDiscountPercentage(data.discountPercentage);
            setMaxDiscountAmount(data.maxDiscountAmount);
            setFixedAmount(data.fixedAmount);
            setRequireQuantity(data.requireQuantity);
            setRewardQuantity(data.rewardQuantity);
            setRequireOrderAmount(data.requireOrderAmount);
            setRequireCustomerPoint(data.requireCustomerPoint);
            setSelectedProducts(data.applicableProducts);
            setGifts(data.giftItems);
    ...
    return <>
        {loading === true ? <div className="fixed top-0" >Loading</div> : (<div className="flex flex-col gap-y-4 h-full">
            <div>
            ...
            <div className="flex gap-4 h-full w-full mb-4">
                <div className="flex flex-col gap-y-6 p-4 bg-white w-1/2 rounded-lg">
                    ...
                    <div className="flex gap-x-6 px-4 items-center">
                        ...
                        <IncrementalField initialValue={maxUsage} onQuantityChange={(e) => setMaxUsage(Number(e.target.value))} />
                    </div>
                    <div className="flex gap-x-6 px-4 items-center">
                        <TextField value={description} onChange={(e) => setDescription(e.target.value)} label="Description" height="200px" placeholder="Promotion description (Optional)" />
                    </div>
                </div>
                <div className="flex flex-col gap-y-6 p-4 bg-white w-1/2 rounded-lg">
                    <div>
                    ...
                    <TypePicker
                        ...
                        options={availableDiscountTypes ? availableDiscountTypes.map(type => ({ label: type, value: type })) : []}
                    />
                        ...
                        }
                        {
                            ...
                            (
                                <div className="flex gap-x-4">
                                    <p className="text-sm font-semibold text-gray-800 w-[30%]">Fixed amount</p>
                                    <MoneyField suffixIcon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm3 0h.008v.008H18V10.5Zm-12 0h.008v.008H6V10.5Z" />
                                    </svg>
                                    } 
                                    value={fixedAmount} 
                                    onChange={(e) => setFixedAmount(Number(e.target.value))} width="30%" />
                                </div>
                            )
                        }
                        {
                    ...
                    <div className="flex justify-center">
                        <RoundedButton width="30%" label="Save" onClick={() => handleEditPromotion()} />
                    </div>
                </div>
            </div>
        </div>)}
