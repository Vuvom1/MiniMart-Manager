...
function AddImport() {
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const activeSuppliers = data.filter(
        (supplier: any) => supplier.status === SupplierStatus.ACTIVE
      );
  ...
  useEffect(() => {
    const newTotalQuantity = importDetails.reduce(
      (acc, product) => acc + product.quantity,
      0
    );
    const newTotalPrice = importDetails.reduce(
      (acc, product) => acc + product.importPrice * product.quantity,
      0
    );
  ...
  const handleUpdateDetail = (products: Product[]) => {
    products.forEach((product) => {
      if (
        !importDetails.some((selected) => selected.product._id === product._id)
      ) {
  ...
  const handleTotalPriceChange = (id?: string, totalPrice?: number) => {
    if (id && totalPrice)
      setImportDetails((prev) =>
        prev.map((detail) =>
          detail.product._id === id
            ? { ...detail, totalImportPrice: totalPrice }
            : detail
        )
      );
  };
        ...
        prevDetails.map((detail) =>
          detail.product._id === addedProduct._id
            ? {
                ...detail,
                importPrice,
                totalImportPrice: importPrice * detail.quantity,
              }
            : detail
        )
      );
          ...
          <div className="flex flex-col flex-auto gap-y-2 overflow-y-auto max-h-full py-4">
            {importDetails.map((detail) => (
              <ImportProductHorizontalCard
                ...
                onPriceChange={(price) =>
                  handlePriceChange(detail.product, price)
                }
                onTotalQuantityChange={(quantity) =>
                  handleTotalQuantityChange(detail.product._id, quantity)
                }
                onTotalPriceChange={(totalPrice) =>
                  handleTotalPriceChange(detail.product._id, totalPrice)
                }
              />
            ))}
            {importDetails.length == 0 && (
              <p className="text-center text-gray-400">
                There is no product, please add products for adding new import!
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col px-8 mt-4">
            ...
            <div className="flex justify-between">
              ...
              <p className="font-medium">
                $
                {isNaN(totalPrice) || !Number.isFinite(totalPrice)
                  ? 0
