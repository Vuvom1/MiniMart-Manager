...
function AddImport() {
  ...
  useEffect(() => {
    const newTotalQuantity = importProducts.reduce(
      (acc, product) => acc + product.quantity,
      0
    );
    const newTotalPrice = importProducts.reduce(
      (acc, product) => acc + product.totalImportPrice,
      0
    );
  ...
  const handleSelectProduct = (product: any) => {
    setImportProducts((prev) => [
      ...prev,
      { ...product, quantity: 1, totalImportPrice: product.price },
    ]);
  ...
  const handleTotalPriceChange = (id: string, totalPrice: number) => {
    setImportProducts((prev) =>
      prev.map((product) =>
        product._id === id
          ? { ...product, totalImportPrice: totalPrice }
          : product
      )
    );
  };
            ...
            {importProducts.map((product) => (
              <ImportProductHorizontalCard
                ...
                onTotalQuantityChange={(quantity) =>
                  handleTotalQuantityChange(product._id, quantity)
                }
                onTotalPriceChange={(totalPrice) =>
                  handleTotalPriceChange(product._id, totalPrice)
                }
              />
            ))}
          </div>
