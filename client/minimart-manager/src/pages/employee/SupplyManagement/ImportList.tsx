          ...
          <div className="overflow-auto flex-auto h-[calc(100vh-350px)] rounded-lg ">
            <table className="min-w-full bg-white text-center rounded-lg shadow-md">
              <thead className="bg-gray-100 sticky top-0 border ">
                <tr>
                  {importsColumnData.map(
                    (column, index) =>
                      column.isCollapsed == true && (
                        <th
                          key={index}
                          className={`sticky ${
                            column.type == ColumnType.ID ? "pl-8" : ""
                          } px-4  py-2 text-gray-400 font-semibold ${
                  ...
                  <th className="sticky px-4 py-2 text-gray-400 font-semibold">
                    Detail
                  </th>
                </tr>
              </thead>
                            ...
                            {column.type === ColumnType.ID ? (
                              <div className="justify-start gap-x-2 flex">
                                <button
                                  ...
                                  onClick={() =>
                                    handleCopyId(item[column.field])
                                  }
                                >
                                  <svg
                                    xmlns="http://www.w3.org/2000/svg"
                              ...
                              <div className="justify-center flex">
                                <StatusBadge
                                  ...
                                  mapping={importStatusColorMapping}
                                />
                              </div>
                            ) : column.type === ColumnType.DATE ? (
                              <p className="text-gray-700 text-left">
                                {timeUtil.formatDateToDayMonthYear(
                                  item[column.field]
                                )}
                              </p>
                            ) : (
                              <p className="text-gray-700 text-start">
                                {" "}
                                {NestedValueUtil.getNestedValue(
                                  item,
                                  column.field
                                ) ?? "N/A"}
                              </p>
